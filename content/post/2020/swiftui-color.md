---
title: 'SwiftUI Color'
date: 2020-02-22T13:53:36+10:00
draft: true
description: ''
tags: ['swift', 'swiftui', 'color']
---

As developers, we are used to thinking of color as a numeric way to specify a particular tint. But in SwiftUI, `Color` - like almost everything else - is actually a `View` in its own right. This has two very interesting aspects: how do we use a view to specify a color and how can we use the fact that `Color` is `View`.
...
The `foregroundColor` modifier expects a `Color` so there was no need to tell it that the parameter is a `Color` and I was able to use one of the standard colors. Command-click on `.red` in the code and select "Jump to Definition". This lists the available pre-defined colors. As well a set of basic colors and a clear option, there are two extra entries: primary & secondary. And if you scroll back up the page a bit, you will see another one: accentColor.

Choosing one of the preset options is the easiest way to select a color and it gives you this auto-switching for free. But try this:

In your code, type `let uiColor = UIColor.` and have a look at the auto-complete suggestions. Scrolling past the various `init` methods, you will see that `UIColor` has a lot more options than `Color`. There are a bunch of colors, a section of system colors and a lot of semantic colors like placeholderText, secondarySystemBackground.
...
Go to Assets.xcassets and click the plus button at the bottom of the list of assets. For a new project this list will only contain the AppIcon. You will get a new asset called "Color" and you can double-click the name to edit it to something that makes sense to you.

{{< img_border >}}

![Add Color Set][i2]

While naming it after the color may seems logical, I prefer to think about the use cases for this color and set the name to something like "cardBackground" or "alertText".

By default, your new color set will contains a single color block. Click in the color block itself and then you will be able to edit the color in the Attributes Inspector. If you click the "Content" popup, you will get access to all the system colors, and you can also select any color space and create a custom color using that color space. Click "Color Panel" to access a standard color picker if you need it. And change the input method to whatever suit the colors you have.
...
Now your color set has two blocks and you can change the Dark one to whatever color this should be in dark mode.
...
Here is the code for my `AlertView` using two color sets. The `Text` is wrapped in a `NavigationView` because that seems to be the only way to get the canvas to preview the different color schemes.

Some people may object to using "magic strings" to specify the color sets but I feel that so long as the Library is used to insert the color names automatically, there is very little chance of error. But of course you could always make an enum of color name constants and use these instead.
...
Apple has done something very clever with the preset colors: they adapt automatically to the environment so that they work with dark mode or light mode just like your own Color Sets can.

I wrote a test view to loop through all the preset colors and display their descriptions in their own color.
...
When choosing a UIColor or NSColor, it is best to use the system versions, so `systemBlue` instead of `blue` as this gives the dark/light variants that we just saw.
...
Resume the canvas preview and you will see the color fill except for the safe areas top and bottom.
...
To be honest, in SwiftUI, you would probably use a `Rectangle` instead of a plain `Color` but it is interesting to consider how `Color` could be made to work.
...
I prefer to put the `edgesIgnoringSafeArea` modifier on the `Color` only and not on the entire `ZStack`. That way the contents of the `ZStack` will all stay inside the safe area.
